...
def save_checkpoint(data_name, epoch, epochs_since_improvement, encoder, decoder, encoder_optimizer, decoder_optimizer,
    ...
    filename = 'checkpoint_En_It_Es_01_' + data_name + '.pth.tar'
    ckp_path = os.path.join(output_dir, filename)
    best_ckp_path = os.path.join(output_dir, 'BEST_' + filename)
    torch.save(state, ckp_path)
    # If this checkpoint is the best so far, store a copy so it doesn't get overwritten by a worse checkpoint
    if is_best:
        torch.save(state, best_ckp_path)
        


def first_token_save_checkpoint(data_name, epoch, epochs_since_improvement, encoder, decoder, encoder_optimizer, decoder_optimizer,
                    metrics, is_best, final_args):
    """
    Saves model checkpoint.
