...
def caption_image_beam_search(args, encoder, decoder, image_path, word_map):
    ...
    if args.decoder_mode == "lstm":
        ...
        k_prev_words = torch.LongTensor([[word_map['<en>']] * 52] * k).to(device)  # (k, 52), <en> or <es> or <it>

    # Tensor to store top k sequences; now they're just <start>
    seqs = torch.LongTensor([[word_map['<en>']]] * k).to(device)  # (k, 1), <en> or <es> or <it>
...
if __name__ == '__main__':
    ...
    parser.add_argument('--img', '-i', default="./test2014/COCO_test2014_000000144344.jpg", help='path to image file or folder')
    parser.add_argument('--checkpoint', '-m', default="/./BEST_checkpoint_En_It_Es_01_coco_5_cap_per_img_3_min_word_freq.pth.tar",
                         help='path to model')
    parser.add_argument('--word_map', '-wm', default="./WORDMAP_coco_5_cap_per_img_3_min_word_freq.json",
    ...
    parser.add_argument('--save_img_dir', '-p', default="./save_results/", help='path to save annotated img.')
