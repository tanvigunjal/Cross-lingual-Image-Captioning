...
def evaluate_lstm(args):
    ...
    with torch.no_grad():
        for i, (image, caps, caplens, allcaps) in enumerate(tqdm(loader, desc="EVALUATING AT BEAM SIZE " + str(beam_size))):
            ...
            # Tensor to store top k previous words at each step; now they're just <en> or <es> or <it>
            k_prev_words = torch.LongTensor([[word_map['<en>']]] * k).to(device)  # [k, 1]
...
def evaluate_transformer(args):
    ...
    with torch.no_grad():
        for i, (image, caps, caplens, allcaps) in enumerate(tqdm(loader, desc="EVALUATING AT BEAM SIZE " + str(beam_size))):
            ...
            # <en> or <es> or <it> is enough for the first step
            k_prev_words = torch.LongTensor([[word_map['<en>']]*52] * k).to(device)  # (k, 52)
            # Tensor to store top k sequences; now they're just <start>
            seqs = torch.LongTensor([[word_map['<en>']]] * k).to(device)  # (k, 1)
            # Tensor to store top k sequences' scores; now they're just 0
            top_k_scores = torch.zeros(k, 1).to(device)
            # Lists to store completed sequences and scores
            complete_seqs = []
           ...
           # Check if the sequence is empty
            if not complete_seqs:
                # If empty, increment a counter for empty sequences
                empty_sequence_count += 1
            else:
                # Find the index of the sequence with the best score
                indices = complete_seqs_scores.index(max(complete_seqs_scores))
                seq = complete_seqs[indices]
            # References
            img_caps = allcaps[0].tolist()
            ...
            hypotheses.append([w for w in seq if w not in {word_map['<start>'], word_map['<end>'], word_map['<pad>']}])
            # join sentence
            sentence = " ".join([rev_word_map[w] for w in sentence])
...
if __name__ == '__main__':
    ...
    parser.add_argument('--checkpoint', default="./BEST_checkpoint_En_It_Es_01_coco_5_cap_per_img_3_min_word_freq.pth.tar",
    ...
    print("{} - beam size {}: BLEU-1 {} BLEU-2 {} BLEU-3 {} BLEU-4 {} ROUGE_L {} CIDEr {}".format
